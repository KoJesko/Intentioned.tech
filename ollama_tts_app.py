...
def check_and_install_dependencies():
    ...
    required_packages = {"ollama": "ollama", "edge_tts": "edge-tts", "pygame": "pygame"}
    ...
    try:
        ...
        print(
            "Please try running the script again or manually install the packages.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def synthesize_and_process_audio(
    text, speaker_voice, seed, output_path=None, compile_tts=False
):
    """
    ...
    if not text.strip():
        print(
            "Ollama returned an empty response. Nothing to synthesize.", file=sys.stderr
        )
        return
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Query Ollama and synthesize the response with realistic TTS."
    )
    parser.add_argument(
        "prompt", type=str, help="The prompt to send to the Ollama model."
    )
    parser.add_argument(
        "--model", type=str, default="llama3.1:latest", help="The Ollama model to use."
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="A seed for reproducible output."
    )
    parser.add_argument(
        "--output_path",
        type=str,
        help="Optional. Path to save the generated audio as a .mp3 file.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Enable verbose output to see performance metrics like tokens/sec.",
    )
    parser.add_argument(
        "--voice",
        type=str,
        help="Optional. Voice to use (e.g., en-US-AriaNeural, en-US-GuyNeural).",
    )
    ...
    try:
        ...
        print(
            f"Error: Could not connect to Ollama or find model '{args.model}'.",
            file=sys.stderr,
        )
        print(
            "Please ensure Ollama is running and the model is pulled.", file=sys.stderr
        )
        sys.exit(1)
    ...
    synthesize_and_process_audio(
        ollama_response, args.voice, args.seed, args.output_path, False
    )
