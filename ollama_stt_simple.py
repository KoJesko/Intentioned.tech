...
def check_and_install_dependencies():
    ...
    if packages_to_install:
        ...
        for module_name, package_name in packages_to_install:
            ...
            if not install_package(package_name):
                print(
                    f"Failed to install {package_name}. Please install manually.",
                    file=sys.stderr,
                )
                if module_name == "pyaudio":
                    print(
                        "For PyAudio on Windows, try: pip install --upgrade pip setuptools wheel",
                        file=sys.stderr,
                    )
    ...
    try:
        ...
        try:
            ...
            print(
                "PyAudio import failed. Some microphone features may not work.",
                file=sys.stderr,
            )
            pyaudio = None
        ...
        try:
            ...
            print(
                "Pydub import failed. Some audio processing features may not work.",
                file=sys.stderr,
            )
            AudioSegment = None
...
def record_audio_simple(duration=5):
    ...
    try:
        ...
        print(
            "Please check if a microphone is connected and accessible.", file=sys.stderr
        )
        return None
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            try:
                ...
                if (
                    cuda_result.returncode == 0
                    and cuda_result.stdout.strip()
                    and cuda_result.stdout.strip() != "N/A"
                ):
            ...
            driver_version = (
                result.stdout.strip().split(",")[1].strip()
                if "," in result.stdout
                else ""
            )
            if driver_version:
...
def check_docker_support():
    ...
    docker_info = {"available": False, "running": False, "version": None}
...
def main():
    ...
    parser.add_argument(
        "--voice", type=str, help="Voice to use for TTS (e.g., female_us, male_uk)"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--runtime",
        type=str,
        choices=["auto", "cpu", "cuda", "nvidia-studio", "nvidia-gaming"],
        default="auto",
        help="Runtime to use (default: auto)",
    )
    parser.add_argument(
        "--gpu_info", action="store_true", help="Show GPU information and exit"
    )
